...
// https://www.figma.com/file/mHQmhfvLiwmIRXkHlFDTyn/CPU_ICMC-team-library?node-id=415%3A589
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case MOV:
					...
					// -----------------------------           
                    if(pega_pedaco(IR,1,0) == 0){
                        selM4 = ry;
                        selM2 = sM4;
                        LoadReg[rx] = 1;
                    }else if(pega_pedaco(IR,1,0) == 1){
                        selM2 = sSP;
                        LoadReg[rx] = 1;                       
                    }else{
                        selM4 = rx;
